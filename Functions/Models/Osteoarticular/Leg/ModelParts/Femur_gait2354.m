...
%Centre de masse
CoM_femur=(k*Mirror*[0; -0.17; 0]); % dans le rep�re femur
...
Human_model(incr_solid).mother=s_mother;                 % mother : d�finit en fonction du pt d'attache
Human_model(incr_solid).a=[0 0 1]';                    % rotation /z
Human_model(incr_solid).joint=1;                        % liaison pivot
Human_model(incr_solid).limit_inf=-pi/4;               % but�e articulaire inf�rieure
Human_model(incr_solid).limit_sup=2*pi/3;                % but�e articulaire sup�rieure
Human_model(incr_solid).ActiveJoint=1;
Human_model(incr_solid).Visual=0;
Human_model(incr_solid).m=0;                           % masse de r�f�rence
Human_model(incr_solid).b=pos_attachment_pt;              % position du point d'attache par rapport au rep�re parent                     A MODIFIER
...
Human_model(incr_solid).ClosedLoop=[];                 % si solide de fermeture de boucle : {num�ro du solide i sur lequel est attach� ce solide ; point d'attache (rep�re du solide i)}
