function [OsteoArticularJoint]= XSens_Pelvis(OsteoArticularJoint,tree,Mass,AttachmentPoint,pose)
...
[~,~,num_solid]=intersect('Pelvis',{points.label});
anatomicalpoints = points(num_solid).points.point;
Pelvis_position_set = cell(numel(anatomicalpoints),2);
FieldNames = fieldnames(anatomicalpoints);
Pos_points = eval(['{anatomicalpoints.' FieldNames{2} '};']);
for i = 1:numel(anatomicalpoints)
    Pelvis_position_set(i,:) = {['Pelvis_' anatomicalpoints(i).label], rotm' * Pos_points{i}'};
end

%% Inertial parameters

[~,~,num_pos1]=intersect('pHipOrigin',{anatomicalpoints.label});
[~,~,num_pos2]=intersect('jL5S1',{anatomicalpoints.label});

Length_Pelvis=norm(Pos_points{num_pos1}-Pos_points{num_pos2});
Mass_Pelvis = 0.1117*Mass;
[I_Pelvis]=rgyration2inertia([61.5 55.1 58.7 0 0 0], Mass_Pelvis, [0 0 0], Length_Pelvis);
c = ((1-61.15)*Pelvis_position_set{num_pos2,2})/100;

for i = 1:numel(anatomicalpoints)
    Pelvis_position_set{i,2} = Pelvis_position_set{i,2} - c;
end
...
%% Pelvis
    ...
    OsteoArticularJoint(incr_solid).m=Mass_Pelvis;
    OsteoArticularJoint(incr_solid).b=pos_attachment_pt;
    OsteoArticularJoint(incr_solid).I=[I_Pelvis(1) I_Pelvis(4) I_Pelvis(5); I_Pelvis(4) I_Pelvis(2) I_Pelvis(6); I_Pelvis(5) I_Pelvis(6) I_Pelvis(3)];
    OsteoArticularJoint(incr_solid).c= c;
    OsteoArticularJoint(incr_solid).anat_position=Pelvis_position_set;
    OsteoArticularJoint(incr_solid).Visual=1;
    OsteoArticularJoint(incr_solid).u=[];
    OsteoArticularJoint(incr_solid).theta=[];
