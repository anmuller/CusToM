function [OsteoArticularJoint]= XSens_UpperLeg(OsteoArticularJoint,tree,Signe,Mass,AttachmentPoint,pose)
% Addition of an upperleg model
%
%   INPUT
...
list_solid={'UpperLeg_J1' 'UpperLeg_J2' 'UpperLeg'};
...
[~,~,num_solid]=intersect([SigneC 'UpperLeg'],{points.label});
anatomicalpoints = points(num_solid).points.point;
UpperLeg_position_set = cell(numel(anatomicalpoints),2);
FieldNames = fieldnames(anatomicalpoints);
Pos_points = eval(['{anatomicalpoints.' FieldNames{2} '};']);
for i = 1:numel(anatomicalpoints)
    UpperLeg_position_set(i,:) = {[SigneC 'UpperLeg_' anatomicalpoints(i).label], rotm'*Pos_points{i}'};
end

%% Inertial parameters

[~,~,num_pos1]=intersect(['j' SigneC 'Hip'],{anatomicalpoints.label});
[~,~,num_pos2]=intersect(['j' SigneC 'Knee'],{anatomicalpoints.label});

Length_UpperLeg=norm(Pos_points{num_pos1}-Pos_points{num_pos2});
Mass_UpperLeg = 0.1416*Mass;
[I_UpperLeg]=rgyration2inertia([32.9 32.9 14.9 0 0 0], Mass_UpperLeg, [0 0 0], Length_UpperLeg, Signe);  
c=(40.95*UpperLeg_position_set{num_pos2,2})/100;

for i = 1:numel(anatomicalpoints)
    UpperLeg_position_set{i,2} = UpperLeg_position_set{i,2} - c;
end
...
%% UpperLeg
    ...
    OsteoArticularJoint(incr_solid).child=s_UpperLeg_J2;                   
    ...
    OsteoArticularJoint(incr_solid).name=[SigneC name];   
    OsteoArticularJoint(incr_solid).sister=0;                
    OsteoArticularJoint(incr_solid).child=s_UpperLeg;                   
    OsteoArticularJoint(incr_solid).mother=s_UpperLeg_J1;           
    ...
    OsteoArticularJoint(incr_solid).mother=s_UpperLeg_J2;
    OsteoArticularJoint(incr_solid).a=[0 0 1]';
    OsteoArticularJoint(incr_solid).joint=1;
    OsteoArticularJoint(incr_solid).ActiveJoint=1;
    OsteoArticularJoint(incr_solid).m=Mass_UpperLeg;
    OsteoArticularJoint(incr_solid).b=[0 0 0]';
    ...
    OsteoArticularJoint(incr_solid).anat_position=UpperLeg_position_set;
