function [external_forces] = addPlatformForces_WithoutCoP(external_forces, Solid, p, R, F)
...
%% Calcul de l'effort résultant (computation of resulting forces)

FR0=zeros(nb_frame,3);
if iscell(p) == 1 % plateforme mobile (mobile plateform)
    for i=1:nb_frame
       FR0(i,:)=(R{i}*F(i,1:3)')';    
    end
...
%% Ajout d'un domaine pour le visuel (adding a visual domain)

% on initialise le domaine Visual si il n'existe pas encore
...
for i=1:numel(external_forces) % pour chaque frame
    if iscell(p) == 1 % plateforme mobile
        ...
        CoP = cross(F(i,1:3)',F(i,4:6)')/(norm(F(i,1:3))^2);
        CoP = CoP - (CoP(3)/F(i,3))*F(i,1:3)'; % point sur l'axe avec z=0;
        pt_origin=p+R*CoP;
    end
    pt_end=FR0(i,:)';
    external_forces(i).Visual = [external_forces(i).Visual [pt_origin;pt_end]];
