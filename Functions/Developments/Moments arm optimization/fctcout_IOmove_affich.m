function diff=fctcout_IOmove_affich(x,BiomechanicalModel,num_muscle,Regression,nb_points,involved_solids,num_markersprov,lambda)
...
title(["Fct coût moment, " BiomechanicalModel.Muscles(num_muscle).name,liste_noms])
legend("Actuelle","Ce quon veut atteindre")
ylabel("Moment arm (m)");

subplot(1,2,2)
plot([ x(1:3) ; x(end-2:end)],'o')
hold on
plot(zeros(6,1),'*')
title(["Fct coût IO, " BiomechanicalModel.Muscles(num_muscle).name,liste_noms])
legend("Actuelle","Ce quon veut atteindre")
ylabel("Position (m)");
