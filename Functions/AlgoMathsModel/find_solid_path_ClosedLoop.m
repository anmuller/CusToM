function [solid_path]=find_solid_path_ClosedLoop(Human_model,i,k)
...
% If the path between both solids is a lace on tree
    ...
    if i>k % Starting from smallest solid increment
        solid_1_path_to_ancestor = solid_1_root_path(solid_1_ancestor_ind:end);
        solid_2_path_from_ancestor = sort(solid_2_root_path(solid_2_ancestor_ind+1:end),'descend');
        solid_path = [solid_2_path_from_ancestor solid_1_path_to_ancestor];
    else
        solid_1_path_to_ancestor = sort(solid_1_root_path(solid_1_ancestor_ind:end),'descend');
        solid_2_path_from_ancestor = solid_2_root_path(solid_2_ancestor_ind+1:end);
        solid_path = [solid_1_path_to_ancestor solid_2_path_from_ancestor];
    end  
end
end
