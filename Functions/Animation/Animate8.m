...
%     elseif ~isfield(BiomechanicalModel.GeometricalCalibration,'k_markers') && isfield(BiomechanicalModel.GeometricalCalibration,'k_calib')
%         k_calib = BiomechanicalModel.GeometricalCalibration.k_calib;
%         k = (ModelParameters.Size/1.80)*k_calib;
%     else
%         k = (ModelParameters.Size/1.80);
%     end
%     bonepath=which('GeometricalCalibration');
%     bonepath = fileparts(bonepath);
%     bonepath = [bonepath '\..\..\BoneScaling\'];
%     for ii=find([Human_model.Visual])
%         if isfield(Human_model,'Geometry') ...
%                 && strcmp(Human_model(ii).Geometry,'TLEM')
%             
%             load(fullfile(bonepath,'TLEM',...
%                 Human_model(ii).name))
%             
%             is=num2str(ii);
%             eval(['Human_model(' is ').V=' 'k(' is ')*' Human_model(ii).name '.Noeuds;']);
%             eval(['Human_model(' is ').F=' Human_model(ii).name '.Polygones;']);
%         else
%             try
...
for f=f_affich
    ...
    if bone_anim % To do % to concatenate bones;
        for j=find([Human_model_bis.Visual])
            onearray = ones([1,length(Human_model_bis(j).V)]);
            
            X = (Human_model_bis(j).Tc_R0_Ri*...
                [Human_model_bis(j).V';onearray ])';
            if f==f_affich(1)
                hc(j) = gpatch(Human_model_bis(j).F,X(:,1:3),[227 218 201]/255*0.9,'none');
            end
            animStruct.Handles{f}=[animStruct.Handles{f} hc(j)];
            animStruct.Props{f}={ animStruct.Props{f}{:}, 'Vertices'};
